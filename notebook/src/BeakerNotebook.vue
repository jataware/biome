<template>
  <div class="beaker-notebook">
    <header>
      <Toolbar class="toolbar">
        <template #start>
          ...
          <Button outlined size="small" icon="pi pi-angle-down" iconPos="right" class="connection-button"
            @click="toggleContextSelection" :label="selectedKernel" :loading="!activeContext?.slug" />
        </template>
    ...
    <main style="display: flex;">
      ...
      <SideMenu position="left" :show-label="true" highlight="line" :show-tooltip="false">
      ...
      <div class="ide-cells" style="flex: 100;" @keydown="handleKeyboardShortcut">
        ...
        <NotebookControls @run-cell="runCell()" @remove-cell="removeCell" @add-cell="addCell"
        ...
        <div style="flex: 1; display: flex; position: relative;">
          ...
          <div class="cell-container drag-sort-enable" ref="cellsContainerRef">
            <BeakerCell v-for="(cell, index) in session.notebook.cells" :key="cell.id" :class="{
            ...
            }" ref="notebookCellsRef" :index="index" :drag-enabled="isDragEnabled" @move-cell="handleMoveCell"
              @click="selectCell(index)" :cell="cell" @keyboard-nav="handleNavAction"
              @dragstart="handleDragStart($event, cell, index)" @drop="handleDrop($event, index)"
              @dragover="handleDragOver($event, cell, index)" @dragend="handleDragEnd" />
  ...
  <BeakerContextSelection :isOpen="contextSelectionOpen" :toggleOpen="toggleContextSelection"
    @update-context-info="setContext" :contextProcessing="contextProcessing" />
</template>
...
import BeakerCell from 'beaker-vue/components/BeakerCell.vue';
import NotebookControls from 'beaker-vue/components/NotebookControls.vue';
import BeakerAgentQuery from 'beaker-vue/components/BeakerAgentQuery.vue';
import BeakerContextSelection from "beaker-vue/components/BeakerContextSelection.vue";
import BeakerExecuteAction from "beaker-vue/components/BeakerExecuteAction.vue";
import FooterDrawer from 'beaker-vue/components/FooterDrawer.vue';
import LoggingPane from 'beaker-vue/components/LoggingPane.vue';
import BeakerFilePane from "beaker-vue/components/BeakerFilePane.vue";
import ContextTree from "beaker-vue/components/ContextTree.vue";
import PreviewPane from "beaker-vue/components/PreviewPane.vue";
import SvgPlaceholder from 'beaker-vue/components/SvgPlaceholder.vue';
import SideMenu from "beaker-vue/components/SideMenu.vue";

import DataSourceBrowser from './DataSourceBrowser';

import SideMenuPanel from "beaker-vue/components/SideMenuPanel.vue";
import { arrayMove, capitalize } from 'beaker-vue/util';

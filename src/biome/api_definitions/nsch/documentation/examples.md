...
## Example 4: Analyzing trends in childhood asthma and environmental factors over time (2016-2023) using NSCH data. This example demonstrates how to load multiple years of data, harmonize variables across years, and analyze the relationship between asthma prevalence and environmental factors like smoke exposure, housing insecurity, and food insecurity. It includes state-level comparisons and creates a composite environmental risk score.
...
## Example 5: Analyzing trends in childhood asthma and environmental factors over time (2016-2023) using NSCH data. This updated example demonstrates how to load multiple years of data, harmonize variables across years, and analyze the relationship between asthma prevalence and environmental factors. It includes state-level comparisons and creates a composite environmental risk score. This version uses DataFrame.copy() to avoid pandas/df fragmentation warnings, but may use more memory for large datasets. If you run into memory issues, try using Example 4 above instead.

```
# Example: Analyzing Trends in Childhood Asthma and Environmental Factors Over Time (2016-2023)
# This example demonstrates how to load multiple years of NSCH data, harmonize variables,
# and analyze trends in asthma prevalence and its relationship with environmental factors.
...
def load_nsch_data(year):
    ...
    # Add year column and other derived columns
    df['year'] = year
    df['FIPSST_clean'] = df['FIPSST'].astype(str).str.strip("b'").str.strip("'")
    df['has_asthma'] = np.where(df['K2Q40A'] == 1, 1, 0)
...
state_trends = []
for year, df in nsch_data_by_year.items():
    # Create a copy of the subset we need to avoid fragmentation warnings
    state_df = df.copy()
    
    # Add state names
    state_df['State'] = state_df['FIPSST_clean'].map(fips_to_state)
    
    # Calculate asthma rates by state
    state_asthma = state_df.groupby('State')['has_asthma'].mean().reset_index()
    state_asthma['Year'] = year
...
# Create a risk score based on the number of environmental risk factors
# For the most recent year (2023)
recent_data = nsch_data_by_year[2023].copy()  # Create a copy to avoid fragmentation
